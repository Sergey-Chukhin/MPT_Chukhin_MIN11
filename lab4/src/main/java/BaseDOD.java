...
public class BaseDOD {
  ...
  public ArrayList<Display> getAllDisplays() {
    session.beginTransaction();
    ArrayList<Display> result =
        (ArrayList<Display>) session.createQuery("FROM Display", Display.class).getResultList();
    session.getTransaction().commit();
    return result;
  }

  public ArrayList<Display> getFramesFromOneStorageAndVendor(Storage storage, Vendor vendor) {
    ArrayList<Display> result = (ArrayList<Display>) session
        .createQuery("FROM Display WHERE storage.id = :storage_id and vendor.id = :vendor_id",
            Display.class)
  ...
  public ArrayList<Object> getAllObjects() {
    ...
    result.addAll(session.createQuery("FROM Display", Display.class).getResultList());
